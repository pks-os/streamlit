...
/**
 * When original -> Streamlit does not do any extra processing on the colors
 *
 * When opacity -> Streamlit reduces the opacity of the unselected items
 *
 * When themed -> Streamlit sets the unselected items to the theme's gray20
 *      color and selected items to the theme's primary color
 *
 * When gray -> Streamlit sets the unselected items to the theme's gray20 color
 *      and maintains the original color of the selected items
 */
export type LayerSelectionColorMode =
  | "original"
  | "opacity"
  | "themed"
  | "gray"
...
export const getContextualFillColor = ({
  ...
  isSelected: boolean
  layerSelectionColorMode: Exclude<LayerSelectionColorMode, "original">
  ...
  if (isSelected) {
    ...
    switch (layerSelectionColorMode) {
      case "opacity":
      case "gray":
        // Return its original color at selected opacity
        return (
      ...
      case "themed":
        // Return the themed selected color
        return [
          selectedColor[0] || 0,
          selectedColor[1] || 0,
          selectedColor[2] || 0,
      ...
      default:
        assertNever(layerSelectionColorMode)
    }
  }

  switch (layerSelectionColorMode) {
    case "opacity": {
      // Return the original color at the specified opacity
      return (
        getOriginalColorWithAppliedOpacity({
    ...
    case "themed":
    case "gray":
      // Return the themed unselected color at the specified opacity
      return [
        unselectedColor[0] || 0,
        unselectedColor[1] || 0,
        unselectedColor[2] || 0,
  ...
  throw new Error("Could not determine fill color for object")
}
